<template>
	<view>
		...
		<block v-for="(item, index) in record" :key="index">
			<view class="box">
				<view class="box-top">
					<text v-if="item.status == '0'" style="color: #7f7f7f;">审核中</text>
					<text v-if="item.status == '1'" style="color: #55B5FF;">申请成功，申请金额已退还到您的账户。</text>
					<view class="" v-if="item.status == '2'">
						<text style="color: #FF4D4D;">申请失败，拒绝理由如下：</text>
						<view class="">
							<text style="color: #7f7f7f;">{{ item.is_cause }}</text>
						</view>
						
					</view>
				</view>
				<view class="empty"></view>
				<view class="record">申请记录</view>
				<view class="sum">
					交易编号：
				...
				<view class="time u-p-b-10">{{ item.add_time }}</view>
			</view>
		</block>
	...
	export default {
		...
		methods:{
			getrecord(){
				...
				console.warn(params);
				this.$request('/applet/order/earnestApplication/earnestApplicationList', params).then(res => {
